...
1. El usuario realiza una acción en el navegador (HTML + CSS). El cliente envía una petición HTTP con el token JWT incluido en el encabezado.
2. Spring Security intercepta la petición, valida el token y verifica los permisos del rol antes de permitir el acceso al Controller.
3. El Controller recibe la petición, extrae los parámetros y llama al método correspondiente en la capa Service.
4. El Service aplica las reglas de negocio, ejecuta las validaciones necesarias y delega la persistencia al Repository.
5. El Repository construye la consulta SQL, la ejecuta contra MySQL y devuelve los datos como objetos Entity.
6. El Service transforma los datos en un DTO y lo devuelve al Controller, que lo serializa en JSON y responde al cliente.
